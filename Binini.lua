...
function Tween(targetCFrame)
    ...
    local speed = _G.TweenSpeed or 180
...
v48:SetValue("Stand");  -- Mặc định là chế độ "Stand"

-- Cập nhật giá trị FarmMode khi dropdown thay đổi
v48:OnChanged(function(Mode)
    FarmMode = Mode;  -- Đảm bảo cập nhật đúng giá trị của FarmMode
end);

-- Spawn task để thực hiện hành động dựa trên FarmMode
spawn(function()
    while wait() do
        if FarmMode == "Stand" then
            -- Chế độ đứng yên, giữ vị trí cố định
            Pos = CFrame.new(0, 20, 0);
        elseif FarmMode == "Dodge" then
            -- Chế độ né skill, thay đổi vị trí linh hoạt
            if Type == 1 then
                Pos = CFrame.new(0, 20, 0);
            elseif Type == 2 then
...
-- Spawn task để thay đổi loại Type cho việc di chuyển (hoặc né skill)
spawn(function()
    while wait() do
        if FarmMode == "Dodge" then
            Type = 1;
            wait(0.5);
            Type = 2;
            wait(0.5);
...
if Sea3 then
    local v490 = v16.Main:AddSection("Cake Prince Boss");
    local v491 = v16.Main:AddParagraph({
        Title = "Cake Prince Stats",
        Content = ""
    });
    spawn(function()
        while wait() do
    ...
    local v494 = v16.Main:AddToggle("ToggleDoughKing", {
        ...
        Description = "Dought is required in the server.",
        Default = false
    });
...
local TweenSpeedInput = v16.Setting:AddInput("TweenSpeedInput", {
    Title = "Tween Speed",
    Default = "180",
    Placeholder = "",
    Numeric = false,
    Finished = false,
    Callback = function(SpeedInput)
        local number = tonumber(SpeedInput)
        if number then
            _G.TweenSpeed = number
        else
            warn("Invalid Tween Speed input! Defaulting to 180.")
            _G.TweenSpeed = 180
        end
    end
});
 v85 = v16.Setting:AddToggle("ToggleAutoT", {
    Title = "Bật Tộc V3",
    Description = "",
    Default = false
});
...
local v130 = v16.EPS:AddToggle("ToggleEspPlayer", {
    Title = "Người Chơi",
    Description = "",
    Default = false
});
...
local v131 = v16.EPS:AddToggle("ToggleEspFruit", {
    Title = "Trái",
    Description = "",
    Default = false
});
...
local v132 = v16.EPS:AddToggle("ToggleEspIsland", {
    Title = "Đảo",
    Description = "",
    Default = false
});
...
local v133 = v16.EPS:AddToggle("ToggleEspFlower", {
    Title = "Hoa",
    Description = "",
    Default = false
});
...
local v134 = v16.EPS:AddToggle("ToggleEspRealFruit", {
...
local v135 = v16.EPS:AddToggle("ToggleIslandMirageEsp", {
    Title = "Đảo Bí Ẩn",
    Description = "",
    Default = false
});
...
------------Full Moon Status-------
    ...
    task.spawn(function()
        while task.wait(2) do -- Kiểm tra mỗi 2 giây để giảm tải
            pcall(function()
                ...
                elseif moonTextureId == "http://www.roblox.com/asset/?id=9709149680" then
                    ...
                    FullMoonStatus:SetDesc("Status: 0%")
                end
            end)
        end
    end)
