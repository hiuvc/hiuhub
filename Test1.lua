...
getgenv().BringRange = 300

-- Tween function (KHÔNG thay đổi size quái)
function TweenEnemyToTarget(enemy, targetCF)
...
task.spawn(function()
    while task.wait() do
        pcall(function()
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    for _, cfg in pairs(magnetConfigs) do
                        if cfg.enabled() and cfg.filter(enemy.Name) then
                            ...
                            if cfg.customDistance then
                                allowed = cfg.customDistance(dist)
                            else
                                allowed = dist <= getgenv().BringRange
                            end
            ...
            --------------------------------------------------------
            -- XỬ LÝ CẶP QUÁI → TÍNH MIDPOINT → TWEEN
            --------------------------------------------------------
            for _, pair in pairs(pairsList) do
                ...
                TweenEnemyToTarget(A.Enemy, midpointCF)
                TweenEnemyToTarget(B.Enemy, midpointCF)
            end
