...
ImageButton.MouseButton1Click:connect(function()
    if game.CoreGui:FindFirstChild("VxezeHub") then
...
function Update:Notify(desc, duration)
    ...
    if tick() - lastNotifyTime < 0.1 then return end -- Giảm cooldown một chút để test
    lastNotifyTime = tick()
    ...
    -- === [FIX QUAN TRỌNG] ===
    -- Đã xóa vòng lặp while wait() gây crash tại đây
    ...
    table.insert(NotificationList, {
        OutlineFrame,
        Desc -- Fix: lưu Desc thay vì title (để check trùng nội dung)
    });
end;
...
function Update:StartLoad()
    ...
    TitleLoader.Text = Config and Config.Title or "Loading..."; -- Safe check
...
function Update:Window(Config)
    -- Thêm safe check nếu người dùng quên nhập Config
    Config = Config or {}
    Config.Title = Config.Title or "Default Hub"
    Config.SubTitle = Config.SubTitle or "v1.0"
    Config.Theme = Config.Theme or ""
    Config.UiTransparency = Config.UiTransparency or 0.1
    Config.Size = Config.Size or UDim2.new(0, 500, 0, 300)
    ...
    BackgroundSettings.BackgroundTransparency = 0.6;
    ...
    function uitab:Tab(text, img)
        ...
        TabButton.MouseButton1Click:Connect(function()
            ...
            (TweenService:Create(TabButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = Config.UiTransparency })):Play();
            ...
            for i, v in next, option do
                ...
                Item.MouseButton1Click:Connect(function()
                    ...
                    for i, v in next, DropScroll:GetChildren() do
                         if v:IsA("TextButton") then
                            local sItem = v:FindFirstChild("SelectedItems");
                            if activeItem == v.Text then
                                 v.BackgroundTransparency = Config.UiTransparency;
                                 v.TextTransparency = 0;
                                 if sItem then sItem.BackgroundTransparency = Config.UiTransparency end
                            else
                                 v.BackgroundTransparency = 1;
                                 v.TextTransparency = 0.5;
                                 if sItem then sItem.BackgroundTransparency = 1 end
                            end;
                        end;
                    end;
                end);
        ...
        function main:Slider(text, min, max, set, callback)
            -- [FIX QUAN TRỌNG] Thêm local để biến không bị Global
            local Value = set; 
            ...
            bar1.Size = UDim2.new((set - min) / (max - min), 0, 0, 4); -- Fix công thức tính width ban đầu
            ...
            UserInputService.InputChanged:Connect(function(Input)
                if Dragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
                     -- Fix tính toán Value dựa trên Mouse Position thay vì bar1.Size để tránh giật
                    ...
                    bar1.Size = UDim2.new(SizeScale, 0, 0, 4); -- Cập nhật Size bằng Scale cho mượt
                    -- circlebar tự đi theo bar1 do parent
                end;
            end);
        end;
